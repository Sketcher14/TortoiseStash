...
constexpr uint16_t GFSize = 256; //8
constexpr uint16_t GeneratingPolynomial = 285; //11
...
static const uint8_t GF256Pow2[] = {
...
static const uint8_t GF256Log2[] = {
...
//static const uint8_t GF8Pow2[] = {
//    0x1, 0x2, 0x4, 0x3, 0x6, 0x7, 0x5, 0x1
//};

//static const uint8_t GF8Log2[] = {
//    0, 0, 1, 3, 2, 6, 4, 5
//};
...
uint8_t GF256::Divide(uint8_t A, uint8_t B)
{
    ...
    return GF256Pow2[PowerDiff];
}

void GF256::GenerateTables()
{
    using namespace GFUtilitylocal;
    ...
    for (int i = 2; i < GFSize - 1; ++i)
    {
        GFPow2_gen[i] = Mul2(GFPow2_gen[i - 1]);
        GFLog2_gen[GFPow2_gen[i]] = i;
    }

    GFPow2_gen[GFSize - 1] = 1;

    QStringList Elements;
    for (int i = 0; i < GFSize; ++i)
        Elements << QString("0x%1").arg(GFPow2_gen[i], 0, 16);

    qInfo().noquote() << Elements.join(", ");
    Elements.clear();

    for (int i = 0; i < GFSize; ++i)
        Elements << QString::number(GFLog2_gen[i]);
